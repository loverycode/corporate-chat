...
describe('AttachmentsService', () => {
  ...
  describe('uploadFile', () => {
    ...
    it('отклоняет Mach-O бинарник, переименованный под безобидное расширение', async () => {
      ...
      const machOHeader = Buffer.from([0xfe, 0xed, 0xfa, 0xcf, 0x00, 0x00, 0x00, 0x00]);
      const file = mockFile({
  ...
  describe('getDownloadFile', () => {
    ...
    it('выбрасывает ошибку NotFoundException, если вложение не найдено', async () => {
      ...
      await expect(service.getDownloadFile('missing', 'user-1')).rejects.toThrow('attachment not found');
  ...
  describe('getThumbnailUrl', () => {
    ...
    it('выбрасывает ошибку если миниатюра недоступна', async () => {
      ...
      await expect(service.getThumbnailUrl('att-1', 'user-1')).rejects.toThrow('Thumbnail not available');
    });
