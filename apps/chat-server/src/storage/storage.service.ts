...
export class StorageService implements OnModuleInit{
    ...
    async uploadFile(key: string, body: Buffer, mimeType: string): Promise<void>{
        await this.client.send(new PutObjectCommand({Bucket: BUCKET, Key: key, Body: body, ContentType: mimeType}),);
    }
    async getDownloadUrl(key: string, expiresSeconds = 300): Promise<string>{
