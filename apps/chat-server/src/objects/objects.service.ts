...
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 минут — верхняя граница по ТЗ

@Injectable()
export class ObjectsService {
    private readonly logger = new Logger(ObjectsService.name);
    private readonly portalApiUrl = process.env.PORTAL_API_URL || 'http://portal-mock:3000';
    private readonly serviceToken = process.env.PORTAL_SERVICE_TOKEN || '';
    private readonly publicUrls = (process.env.PORTAL_PUBLIC_URLS || '').split(',').map((s) => s.trim()).filter(Boolean);
    ...
    extractObjectIds(bodyMd: string): string[] {
        ...
        for (const domain of this.publicUrls) {
            ...
            const pattern = new RegExp(`${escaped}/dashboard/object/([0-9a-fA-F-]{36})`, 'g');
            let match;
    ...
    async resolveObjects(objectIds: string[], forUserId: string): Promise<Map<string, ResolvedObject>> {
        ...
        try {
            ...
            this.logger.warn(`Failed to resolve objects: ${err instanceof Error ? err.message : err}`);
        }
